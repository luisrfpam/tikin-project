...
// QuantumCert Mock
export async function mockQuantumCertMint(voucherId: string) {
  // Simulates QuantumCert /diamond/mintAsset
  // In production: Falcon-512 post-quantum signature via QuantumCert API
  const fakeAssetId = `0x${Array.from({ length: 40 }, () => Math.floor(Math.random() * 16).toString(16)).join('')}`;
  return { quantumcert_asset_id: fakeAssetId, status: 'minted' };
}

// Transfero Mock - instant settlement
export async function mockTransferoSettle(transactionId: string, amount: number) {
  // Simulates Transfero instant settlement callback
