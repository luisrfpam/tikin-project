...
BEGIN
  ...
  -- Categoria principal do lojista funciona como fallback quando
  -- accepted_categories estiver vazia (comportamento de cadastros legados).
  v_fallback_category := public.normalize_category(v_est.category);
  v_normalized_accepted := ARRAY(
  ...
  IF COALESCE(array_length(v_normalized_accepted, 1), 0) = 0
     AND COALESCE(v_fallback_category, '') <> '' THEN
    v_normalized_accepted := ARRAY[v_fallback_category];
  END IF;
  ...
  LOOP
    ...
    v_category := public.normalize_category(v_voucher.rules->>'category');
    IF COALESCE(array_length(v_normalized_accepted, 1), 0) > 0 THEN
      IF v_category IS NULL OR v_category = '' OR NOT (v_category = ANY(v_normalized_accepted)) THEN
        RAISE EXCEPTION 'Categoria % não aceita por este lojista', COALESCE(NULLIF(v_category, ''), '(sem categoria)');
      END IF;
    END IF;
    ...
    VALUES (v_voucher.id, _establishment_id, v_slice_amount, 'confirmed',
            NULLIF(v_category, ''), 'Pagamento em ' || v_est.name)
  ...
  -- Marca cobrança como paga e vincula uma transação para rastreabilidade
  IF _charge_id IS NOT NULL THEN
    UPDATE charges
    SET status = 'paid',
        paid_at = now(),
        updated_at = now(),
        transaction_id = COALESCE(v_tx_ids[1], transaction_id)
    WHERE id = _charge_id;
  END IF;
