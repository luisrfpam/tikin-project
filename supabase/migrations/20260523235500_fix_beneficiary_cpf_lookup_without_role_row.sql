-- Fix beneficiary document login for legacy accounts missing user_roles row.
-- Keep role-aware behavior and only allow this fallback for expected_role = 'beneficiario'.
  ...
  matches AS (
    ...
    WHERE (
      ...
      AND (
        ...
        OR ur.role = _expected_role
        OR (_expected_role = 'beneficiario'::public.app_role AND ur.user_id IS NULL)
      )
    )
    OR (
      d.digits <> ''
